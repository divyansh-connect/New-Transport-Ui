...
import { Wrench, Droplet, MapPin, Plus, Search, Eye, Edit2, Trash2, Download, Users } from 'lucide-react';
...
export const Services = () => {
  ...
  const initialServices = [
    { id: 'WS-01', name: 'Central Maintenance Garage', type: 'workshop', location: 'Zone 4, Sector B', status: 'Active', rating: '4.8', contact: '+1 (555) 0192', amount: '$99.99' },
    { id: 'WS-02', name: 'Heavy Duty Repair Hub', type: 'workshop', location: 'Industrial Park West', status: 'Inactive', rating: '4.2', contact: '+1 (555) 0122', amount: '$99.99' },
    { id: 'OC-01', name: 'Quick Lube & Oil Express', type: 'oil change', location: 'Zone 1, Highway 9', status: 'Active', rating: '4.9', contact: '+1 (555) 0881', amount: '$49.99' },
    { id: 'OC-02', name: 'Express Fleet Fluids', type: 'oil change', location: 'South Terminal', status: 'Active', rating: '4.5', contact: '+1 (555) 0912', amount: '$49.99' },
    { id: 'CL-01', name: 'North Terminal Fleet Hub', type: 'car location', location: 'North Terminal', status: 'Active', rating: '-', contact: 'Internal', amount: '$49.99' },
    { id: 'CL-02', name: 'Downtown Secure Parking', type: 'car location', location: 'Downtown Hub', status: 'Maintenance', rating: '-', contact: 'Internal', amount: '$49.99' },
  ];
  ...
  const [formData, setFormData] = useState({ name: '', location: '', latitude: '', longitude: '', contact: '', status: 'Active', amount: '' });
  ...
  const handleExport = () => {
    if (activeTab === 'driver') {
      const headers = ["Driver ID", "Name", "City", "Mobile Number", "Status"];
  ...
  const handleAddClick = () => {
    setModalMode('add');
    setFormData({ name: '', location: '', latitude: '28.6250', longitude: '77.2180', contact: '', status: 'Active', amount: '$49.99' });
    setIsModalOpen(true);
  };
  ...
  const handleEditClick = (record) => {
    ...
    setFormData({ name: record.name, location: record.location, latitude: record.latitude || '28.6250', longitude: record.longitude || '77.2180', contact: record.contact, status: record.status, amount: record.amount || '$49.99' });
    setIsModalOpen(true);
  };
    ...
    if (modalMode === 'add') {
      ...
      const updated = allServices.map(s => {
        if (s.id === selectedRecord.id) {
          return { ...s, name: formData.name, location: formData.location, latitude: formData.latitude, longitude: formData.longitude, contact: formData.contact, status: formData.status, amount: formData.amount };
        }
        return s;
      });
      ...
      >
        {modalMode === 'view' && selectedRecord && (
          <div className="modal-record-details" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: activeTab === 'driver' ? '1fr' : '1.2fr 0.8fr', gap: '16px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
